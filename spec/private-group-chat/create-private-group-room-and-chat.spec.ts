...
import { login, invitePrivateGroupUser, createBrowserAndPage, createGroupChat, enterRoomFromRoomList } from "../utils";
...
describe("test two person enter public room and chat", () => {
    ...
    beforeAll(async () => {
        ...
        await createGroupChat(user1.page, roomName, true);
        await user1.page.waitForTimeout(10 * 1000);

        await invitePrivateGroupUser(user1.page, user2.pubkey);
        await user1.page.waitForTimeout(3 * 1000);

        await enterRoomFromRoomList(user2.page, roomName);
        await user1.page.waitForTimeout(7 * 1000);
        await user2.page.waitForTimeout(7 * 1000);
    }, 360 * 1000);
    afterAll(async () => {
