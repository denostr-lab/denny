...
function RoomProfile({ roomId }) {
  ...
  avatarSrc = isDM ? mx.getUserAvatar(roomId) : avatarSrc;
  ...
  const canChangeAvatar = !isDM && currentState.maySendStateEvent('m.room.avatar', userId);
  const canChangeName = !isDM && currentState.maySendStateEvent('m.room.name', userId);
  const canChangeTopic = !isDM && currentState.maySendStateEvent('m.room.topic', userId);
  ...
  const handleOnSubmit = async (e) => {
    ...
    try {
      ...
      await mx.setRoomProfile(roomId, {
        name,
        about,
        picture: avatarSrc || '',
      });
