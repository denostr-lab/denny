...
function Video({
  ...
  const [blur, setBlur] = useState(true);
  const [blurhashReal, setBlurhashReal] = useState(blurhash);
  const initBlurHash = async () => {
    if (!blurhash && link) {
      const blurhashReal = await initMatrix.roomsInput.loadVideoBlur(link)
      setBlurhashReal(blurhashReal)
    }
  }
  useEffect(() => {
    let unmounted = false;
  ...
  return (
    <div className="file-container">
      ...
      >
        {url === null ? (
          <>
            {blurhashReal && blur && <BlurhashCanvas hash={blurhashReal} punch={1} />}
            {thumbUrl !== null && (
