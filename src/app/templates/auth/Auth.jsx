...
function LoginByNameConfirm(props) {
  ...
  const submitter = async (values, actions) => {
    ...
    // TODO:
    const data = {
      user_id: values.pubkey,
      name: values.nickname || values.npub.slice(5, 9),
      about: '',
      profile_img: '',
      npub: values.npub,
    };
...
function LoginByPriKey() {
  ...
  const submitter = (values, actions) => {
    let prikey = String(values.privatekey || '').trim();
    if (prikey && prikey.length > 0) {
      prikey = Key.toNostrHexAddress(prikey);
      if (!prikey) {
        actions.setErrors({ privatekey: 'invalid private key' });
        actions.setSubmitting(false);
        return;
      }
    }

    const pubkey = Key.getPublicKey(prikey);
    const npub = Key.toNostrBech32Address(pubkey, 'npub');

    localStorage.setItem(cons.secretKey.USER_ID, pubkey);
    localStorage.setItem(cons.secretKey.ACCESS_TOKEN, prikey);
    localStorage.setItem(cons.secretKey.DEVICE_ID, npub);
    localStorage.setItem(cons.secretKey.BASE_URL, window.location.href);
    ...
    <ScrollView invisible>
      <div className="auth__base">
        ...
        <div className="auth-footer">
          <Text variant="b2">
            <a href="https://twitter.com/Denny_nostr" target="_blank" rel="noreferrer">X</a>
          </Text>
          <Text variant="b2">
          ...
          <Text variant="b2">
            <a href="https://denostr.com/" target="_blank" rel="noreferrer">Powered by Denostr</a>
          </Text>
        </div>
      </div>
    </ScrollView>
