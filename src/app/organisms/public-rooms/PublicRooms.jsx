...
function PublicRooms({ isOpen, searchTerm, onRequestClose }) {
  ...
  useEffect(() => {
    if (isOpen) {
      initMatrix.matrixClient.subscribePublicRooms();
      return () => {
        initMatrix.matrixClient.unsubscribePublicRooms();
        updatePublicRooms([]);
        updateSearchQuery({});
        updateIsSearching(false);
        updateNextBatch(undefined);
      ...
      <div className="public-rooms">
        <form className="public-rooms__form" onSubmit={(e) => { e.preventDefault(); searchRooms(); }}>
          <div className="public-rooms__input-wrapper">
            <Input value={searchTerm} forwardRef={roomNameRef} label="Room name or ID" />
