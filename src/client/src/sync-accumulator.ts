...
export class SyncAccumulator {
  ...
  private accumulateJoinState(roomId: string, data: IJoinedRoom, fromDatabase = false): void {
    ...
    currentData._unreadThreadNotifications =
      data[UNREAD_THREAD_NOTIFICATIONS.stable!] ??
...
function setState(
  eventMap: Record<string, Record<string, IStateEvent>>,
  event: IRoomEvent | IStateEvent
): void {
  if (
    (event as IStateEvent).state_key === null ||
    (event as IStateEvent).state_key === undefined ||
    !event.type
  ) {
    return;
  }
