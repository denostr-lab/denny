...
export async function getExistingOlmSessions(
  ...
  const sessions: MapWithDefault<string, Map<string, IExistingOlmSession>> = new MapWithDefault(
    () => new Map()
  );
...
export async function ensureOlmSessionsForDevices(
  ...
  for (const [userId, devices] of devicesByUser) {
    ...
    for (const deviceInfo of devices) {
      ...
      if (!oneTimeKey) {
        log.warn(
          `No one-time keys (alg=${oneTimeKeyAlgorithm}) ` + `for device ${userId}:${deviceId}`
        );
...
async function _verifyKeyAndStartSession(
  ...
  try {
    ...
    logger.error(
      'Unable to verify signature on one-time key for device ' + userId + ':' + deviceId + ':',
      e
    );
    return null;
  }
 ...
 */
export function pkSign(
  obj: object & IObject,
  key: Uint8Array | PkSigning,
  userId: string,
  pubKey: string
): string {
