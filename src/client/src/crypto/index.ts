...
export class Crypto
  extends TypedEventEmitter<CryptoEvent, CryptoEventHandlerMap>
  implements CryptoBackend
{
  ...
  private lastNewSessionForced: MapWithDefault<string, MapWithDefault<string, number>> =
    new MapWithDefault(() => new MapWithDefault(() => 0));
  ...
  public constructor(
    ...
    this.deviceList.on(
      ...
      this.onDeviceListUserCrossSigningUpdated
    );
    this.reEmitter.reEmit(this.deviceList, [
    ...
    this.secretStorage = new SecretStorage(
      baseApis as IAccountDataClient,
      cryptoCallbacks,
      baseApis
    );
  ...
  public async bootstrapSecretStorage({
    ...
    const ensureCanCheckPassphrase = async (
      keyId: string,
      keyInfo: ISecretStorageKeyInfo
    ): Promise<void> => {
      if (!keyInfo.mac) {
    ...
    const signKeyBackupWithCrossSigning = async (
      keyBackupAuthData: IKeyBackupInfo['auth_data']
    ): Promise<void> => {
      if (
    ...
    if (!storageExists && !keyBackupInfo) {
      ...
      const { keyInfo = {} as IAddSecretStorageKeyOpts, privateKey } =
      ...
      const backupKey =
        (await this.getSessionBackupPrivateKey()) || (await getKeyBackupPassphrase?.());
  ...
  public checkIfOwnDeviceCrossSigned(deviceId: string): boolean {
    ...
    return (
      userCrossSigning
        ?.checkDeviceTrust(userCrossSigning, device, false, true)
  ...
  public async setDeviceVerification(
    ...
    if (xsk && xsk.getId() === deviceId) {
      ...
      if (keys && (Object.values(keys).length !== 1 || gotKeyId !== xsk.getId())) {
        throw new Error(
          `Key did not match expected value: expected ${xsk.getId()}, got ${gotKeyId}`
        );
      }
      ...
      if (userId !== this.userId) {
        logger.info(
          'Master key ' + xsk.getId() + ' for ' + userId + ' marked verified. Signing...'
        );
      ...
      if (keys) {
        for (const [keyId, key] of Object.entries(keys)) {
          if (dev.keys[keyId] !== key) {
            throw new Error(
              `Key did not match expected value: expected ${key}, got ${dev.keys[keyId]}`
            );
          }
        }
      }
  ...
  public requestVerification(userId: string, devices?: string[]): Promise<VerificationRequest> {
    ...
    const existingRequest = this.toDeviceVerificationRequests.findRequestInProgress(
      userId,
      devices
    );
    if (existingRequest) {
  ...
  public getEventSenderDeviceInfo(event: MatrixEvent): DeviceInfo | null {
    ...
    if (!claimedKey) {
      logger.warn(
        'Event ' + event.getId() + ' claims no ed25519 key: ' + 'cannot verify sending device'
      );
      return null;
    }
  ...
  private async setRoomEncryptionImpl(room: Room, config: IRoomEncryption): Promise<void> {
    ...
    if (existingConfig) {
      if (JSON.stringify(existingConfig) != JSON.stringify(config)) {
        logger.error(
          'Ignoring m.room.encryption event which requests ' + 'a change of config in ' + roomId
        );
        return;
      }
    }
   ...
   * /keys/changes
   */
  private async evalDeviceListChanges(
  ...
  public async signObject<T extends ISignableObject & object>(obj: T): Promise<void> {
    ...
    userSignatures['ed25519:' + this.deviceId] = await this.olmDevice.sign(
