...
export class SyncApi {
  ...
  private onMarkerStateEvent(
    ...
    if (isValidMsc2716Event) {
      ...
      logger.debug(
        `MarkerState: Ignoring markerEventId=${markerEvent.getId()} in roomId=${
          room.roomId
        } because ` +
  ...
  public async syncLeftRooms(): Promise<Room[]> {
    ...
    const data = await client.http.authedRequest<ISyncResponse>(
      Method.Get,
      '/sync',
      qps as any,
      undefined,
    ...
    const rooms = await Promise.all(
      leaveRooms.map(async (leaveObj) => {
        ...
        room
          .getLiveTimeline()
          .setPaginationToken(leaveObj.timeline.prev_batch, EventTimeline.BACKWARDS);
  ...
  public async recoverFromSyncStartupError(
    savedSyncPromise: Promise<void> | undefined,
    error: Error
  ): Promise<void> {
  ...
  private buildDefaultFilter = (): Filter => {
    const filter = new Filter(this.client.credentials.userId);
    if (
      this.client.canSupport.get(Feature.ThreadUnreadNotifications) !== ServerSupport.Unsupported
    ) {
  ...
  private checkLazyLoadStatus = async (): Promise<void> => {
    ...
    if (shouldClear) {
      ...
      const error = new InvalidStoreError(
        InvalidStoreState.ToggledLazyLoading,
  ...
  private getFilter = async (): Promise<{
    ...
    try {
      filterId = await this.client.getOrCreateFilter(
        getFilterName(this.client.credentials.userId!),
        filter
      );
    } catch (err) {
  ...
  private async processSyncResponse(
    ...
    if (Array.isArray(data.presence?.events)) {
      data
        .presence!.events.filter(noUnsafeEventProps)
    ...
    if (Array.isArray(data.account_data?.events)) {
      const events = data.account_data.events
    ...
    if (
      data.to_device &&
      Array.isArray(data.to_device.events) &&
      data.to_device.events.length > 0
    ) {
      let toDeviceMessages: IToDeviceEvent[] = data.to_device.events.filter(noUnsafeEventProps);

      if (this.syncOpts.cryptoCallbacks) {
        toDeviceMessages = await this.syncOpts.cryptoCallbacks.preprocessToDeviceMessages(
          toDeviceMessages
        );
      }
    ...
    await utils.promiseMapSeries(inviteRooms, async (inviteObj) => {
      ...
      const inviter = room.currentState
    ...
    await utils.promiseMapSeries(joinRooms, async (joinObj) => {
      ...
      if (joinObj.isBrandNewRoom) {
        ...
        if (joinObj.timeline.prev_batch !== null) {
          room
            .getLiveTimeline()
            .setPaginationToken(joinObj.timeline.prev_batch, EventTimeline.BACKWARDS);
        }
    ...
    if (
      ...
      this.syncOpts.cryptoCallbacks?.preprocessUnusedFallbackKeys(
        new Set<string>(unusedFallbackKeys || null)
      );
    }
  }
...
export function _createAndReEmitRoom(
  client: MatrixClient,
  roomId: string,
  opts: Partial<IStoredClientOpts>
): Room {
