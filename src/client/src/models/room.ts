...
export class Room extends ReadReceipt<RoomEmittedEvents, RoomEventHandlerMap> {
  ...
  private async loadMembers(): Promise<{ memberEvents: MatrixEvent[]; fromServer: boolean }> {
    ...
    if (
      rawMembersEvents === null ||
      (this.client.isCryptoEnabled() && this.client.isRoomEncrypted(this.roomId))
    ) {
      fromServer = true;
      rawMembersEvents = await this.loadMembersFromServer();
      logger.log(
        `LL: got ${rawMembersEvents.length} ` + `members from server for room ${this.roomId}`
      );
    }
    const memberEvents = rawMembersEvents
    ...
    inMemoryUpdate
      .then((fromServer) => {
        if (fromServer) {
          ...
          logger.log(
            `LL: telling store to write ${oobMembers.length}` + ` members for room ${this.roomId}`
          );
  ...
  private cleanupAfterLeaving(): void {
    this.clearLoadedMembersIfNeeded().catch((err) => {
      logger.error(
        `error after clearing loaded members from ` + `room ${this.roomId} after leaving`
      );
      logger.log(err);
    });
  }
  ...
  public async refreshLiveTimeline(): Promise<void> {
    ...
    if (!mostRecentEventInTimeline) {
      ...
      // timeline.
      newTimeline = await this.client.getEventTimeline(
        timelineSet,
        mostRecentEventInTimeline.getId()!
      );
    }
   ...
   */
  public getUnreadCountForEventContext(
   ...
   *          for this type.
   */
  public getThreadUnreadNotificationCount(
    threadId: string,
    type = NotificationCountType.Total
  ): number {
   ...
   * @returns
   */
  public setThreadUnreadNotificationCount(
    threadId: string,
  ...
  public async fetchRoomThreads(): Promise<void> {
    ...
    if (Thread.hasServerSideListSupport) {
      ...
      const threadRoots = events.map(this.client.getEventMapper()).sort((eventA, eventB) => {
        ...
        return (
          threadAMetadata.latest_event.origin_server_ts -
  ...
  private async fetchRoomThreadList(filter?: ThreadFilterType): Promise<void> {
    const timelineSet =
      filter === ThreadFilterType.My ? this.threadsTimelineSets[1] : this.threadsTimelineSets[0];
  ...
  public eventShouldLiveIn(
    ...
    if (parentEventId) {
      parentEvent =
        this.findEventById(parentEventId) ?? events?.find((e) => e.getId() === parentEventId);
    }
  ...
  public handleRemoteEcho(remoteEvent: MatrixEvent, localEvent: MatrixEvent): void {
    ...
    logger.debug(
      `Got remote echo for event ${oldEventId} -> ${newEventId} old status ${oldStatus}`
    );
  ...
  public addLiveEvents(
    ...
    let duplicateStrategy: DuplicateStrategy | undefined =
      duplicateStrategyOrOpts as DuplicateStrategy;
    ...
    Object.keys(content).forEach((eventId: string) => {
      Object.keys(content[eventId]).forEach((receiptType: ReceiptType | string) => {
        Object.keys(content[eventId][receiptType]).forEach((userId: string) => {
          ...
          const receiptForMainTimeline =
            !receipt.thread_id || receipt.thread_id === MAIN_ROOM_TIMELINE;
          ...
          if (
            ...
            receipt.ts < this.oldestThreadedReceiptTs
          ) {
    ...
    if (membershipEvent) {
      ...
      if (membership === 'invite') {
        ...
        strippedStateEvents.forEach((strippedEvent) => {
          const existingEvent = this.currentState.getStateEvents(
            strippedEvent.type,
  ...
  private redactVisibilityChangeEvent(event: MatrixEvent): void {
    ...
    const index = visibilityEventsOnOriginalEvent.findIndex(
      (change) => change.getId() === event.getId()
    );
    if (index === -1) {
    ...
    if (index === visibilityEventsOnOriginalEvent.length) {
      ...
      if (index === 0) {
        ...
        const newEvent =
          visibilityEventsOnOriginalEvent[visibilityEventsOnOriginalEvent.length - 1];
