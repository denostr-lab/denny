...
import {
  Thread,
  ThreadEvent,
  EventHandlerMap as ThreadEventHandlerMap,
...
> {
  ...
  public get isThreadRoot(): boolean {
    const threadDetails = this.getServerAggregatedRelation<IThreadBundledRelationship>(
   ...
   */
  public async attemptDecryption(
  ...
  private async decryptionLoop(
    ...
    while (true) {
      ...
      try {
        ...
        const detailedError =
          e instanceof DecryptionError ? (<DecryptionError>e).detailedString : String(e);
  ...
  private setClearData(decryptionResult: IEventDecryptionResult): void {
    ...
    this.encryptedDisabledForUnverifiedDevices =
      decryptionResult.encryptedDisabledForUnverifiedDevices || false;
  ...
  public replacingEventId(): string | undefined {
    const replaceRelation = this.getServerAggregatedRelation<IAggregatedRelation>(
