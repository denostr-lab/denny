...
class Events {
  ...
  handleReactionMessageEvent = (
    ...
    let roomid = null;
    const rootRoomid = event.tags.find(
      (tags) => tags[0] === 'e' && tags[3] === 'root'
    )?.[1] as string;
    if (rootRoomid) {
      roomid = rootRoomid;
    } else {
    ...
    const created_at = event.created_at * 1000;
    const replyEventId = event.tags.find(
      (tags) => tags[0] === 'e' && tags[3] === 'reply'
    )?.[1] as string;
    if (replyEventId) {
