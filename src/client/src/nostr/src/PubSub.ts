 ...
 */
const PubSub = {
    internalSubscriptionsByName: new Map<string, number>(), // 全局的存储
    subscriptionsByName: new Map<string, Set<Sub>>(), // 和全局作用差不多
    subscribedFiltersByName: new Map<string, FiltersWithOptions>(), // 监听的过滤条件 用于重放 监听
    subscriptions: new Map<number, Subscription>(), // 监听的递增
    subscribedPosts: new Set<string>(), // 已经监听的 事件 filter 内部的ids的hex
    subscribedRepliesAndReactions: new Set<string>(), // 监听的 reactin 反应 闪电付费发送
    subscribedProfiles: new Set<string>(), // 监听用户变化
    subscribedKeywords: new Set<string>(), //搜索的关键词
    ...
    subscribeToAuthors: debounce(() => {
        ...
        console.log("subscribe to profiles and contacts of", PubSub.subscribeToNewAuthors.size, "new authors");
