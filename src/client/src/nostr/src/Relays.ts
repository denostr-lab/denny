...
class Relays {
  ...
  initRelays(relays: { url: string, enabled: boolean }[]) {
    this.relays = new Map<string, NostrRelay>(relays.map((relay) => [relay.url, this.relayInit(relay.url, true, { enabled: relay.enabled })]));
    this.searchRelays = new Map<string, NostrRelay>(
  ...
  getDefaultRelays(initRelays?: string[]) {
    return [...new Set([...initRelays || [], ...DEFAULT_RELAYS])].map((url) => ({ url, enabled: true, read: true, write: true }));
  }
