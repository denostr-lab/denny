...
export class MatrixClient extends TypedEventEmitter<EmittedEvents, ClientEventHandlerMap> {
   ...
   */
  public keyBackupKeyFromPassword(
    password: string,
    ...
    try {
      ...
      if (!(await algorithm.keyMatches(privKey))) {
        return Promise.reject(
          new MatrixError({ errcode: MatrixClient.RESTORE_BACKUP_ERROR_BAD_KEY })
        );
      }
      ...
      const res = await this.http.authedRequest<
        IRoomsKeysResponse | IRoomKeysResponse | IKeyBackupSession
      >(Method.Get, path.path, path.queryData, undefined, { prefix: ClientPrefix.V3 });
  ...
  public async deleteKeysFromBackup(
    roomId?: string,
    sessionId?: string,
    version?: string
  ): Promise<void> {
   ...
   * @returns Rejects: with an error response.
   */
  public async getAccountDataFromServer<T extends { [k: string]: any }>(
    eventType: string
  ): Promise<T | null> {
  ...
  public cancelPendingEvent(event: MatrixEvent): void {
    if (
      ![EventStatus.QUEUED, EventStatus.NOT_SENT, EventStatus.ENCRYPTING].includes(event.status!)
    ) {
  ...
  protected updatePendingEventStatus(
    room: Room | null,
    event: MatrixEvent,
  ...
  public getRoomUpgradeHistory(
    ...
    const before = this.findPredecessorRooms(
      currentRoom,
      verifyLinks,
      msc3946ProcessDynamicPredecessor
    );
    const after = this.findSuccessorRooms(
      currentRoom,
      verifyLinks,
      msc3946ProcessDynamicPredecessor
    );
  ...
  private findSuccessorRooms(
    ...
    while (tombstoneEvent) {
      ...
      if (verifyLinks) {
        const predecessorRoomId = successorRoom.findPredecessor(
          msc3946ProcessDynamicPredecessor
        )?.roomId;
  ...
  public async getEventTimeline(
    ...
    eventId: string
  ): Promise<Optional<EventTimeline>> {
  ...
  public async getThreadTimeline(
    timelineSet: EventTimelineSet,
    eventId: string
  ...
  public async getLatestTimeline(timelineSet: EventTimelineSet): Promise<Optional<EventTimeline>> {
    ...
    if (timelineSet.threadListType !== null) {
      ...
      const res = await this.http.authedRequest<IMessagesResponse>(
        Method.Get,
        messagesPath,
        params
      );
      ...
      promise = this.createMessagesRequest(
        ...
        .then((res) => {
          ...
          this.processThreadRoots(
            room,
            timelineEvents.filter((it) =>
              it.getServerAggregatedRelation(THREAD_RELATION_TYPE.name)
            ),
            false
          );
  ...
  public searchRoomEvents(opts: IEventSearchOpts): Promise<ISearchResults> {
    ...
    return this.search({ body: body }).then((res) =>
      this.processRoomEventsSearch(searchResults, res)
    );
  }
   ...
   */
  // XXX: Intended private, used in code
  public processRoomEventsSearch<T extends ISearchResults>(
    searchResults: T,
  ...
  public createFilter(content: IFilterDefinition): Promise<Filter> {
    ...
    return this.http
      .authedRequest<IFilterResponse>(Method.Post, path, undefined, content)
      .then((response) => {
        // persist the filter
  ...
  public async getOrCreateFilter(filterName: string, filter: Filter): Promise<string> {
    ...
    if (filterId) {
      ...
      try {
        ...
        // }
        if (
          (<MatrixError>error).errcode !== 'M_UNKNOWN' &&
  ...
  public async checkTurnServers(): Promise<boolean | undefined> {
    ...
    if (remainingTime > TURN_CHECK_INTERVAL) {
      logger.debug(
        'TURN creds are valid for another ' + remainingTime + ' ms: not fetching new ones.'
      );
  ...
  public async _unstable_getSharedRooms(userId: string): Promise<string[]> {
    ...
    const mutualRoomsSupport = await this.doesServerSupportUnstableFeature(
  ...
  public async relations(
    ...
    const fetchedEventType = eventType
      ? this.getEncryptedIfNeededEventType(roomId, eventType)
      : null;
  ...
  public getIdentityServerUrl(stripProto = false): string | undefined {
    if (
      stripProto &&
      (this.idBaseUrl?.startsWith('http://') || this.idBaseUrl?.startsWith('https://'))
    ) {
  ...
  public getPushRules(): Promise<IPushRules> {
    return this.http
      .authedRequest<IPushRules>(Method.Get, '/pushrules/')
      .then((rules: IPushRules) => {
  ...
  public async publishPublicSetMetadata(
    roomId: string,
    metadata: { name: string; about: string; picture: string }
  ) {
...
export function fixNotificationCountOnDecryption(cli: MatrixClient, event: MatrixEvent): void {
  ...
  const currentHighlightCount = room.getUnreadCountForEventContext(
