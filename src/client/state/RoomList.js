...
class RoomList extends EventEmitter {
  ...
  getOrphanRooms() {
    return [...this.rooms].filter((roomId) => !this.roomIdToParents.has(roomId) && !this.matrixClient.hasLeaveRoom(roomId));
  }
  ...
  roomActions(action) {
    ...
    const actions = {
      ...
      [cons.actions.room.LEAVE]: () => {
        setTimeout(() => {
          this.emit(cons.events.roomList.ROOMLIST_UPDATED);
          navigation.emit(cons.events.navigation.ROOM_SELECTED);
        }, 100);
      },
    };
    actions[action.type]?.();
